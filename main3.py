...
def main(page: ft.Page):
        ...
        if profils:
            ...
            for p in profils:
                def choisir(e, p=p):
                    ...
                    profil["age"] = p["age"]
                    profil["poids"] = p["poids"]
                    profil["taille"] = p["taille"]
                    profil["activites"] = p.get("activites", [])
                    profil["genre"]=["genre"]
                    ecran_choix()
                elements.append(ft.Button(
                    f"{p['prenom']}  —  {p['age']} ans, {p['poids']} kg, {p['taille']} cm",
                    on_click=choisir,
                    bgcolor=BLEU, color="white",
                    style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=30)),
                    width=380, height=50,
    ...
    def ecran_bienvenue():
        ...
        genre = ft.TextField(label="Genre (Homme/Femme)", border_color=BLEU, focused_border_color=BLEU, width=300)
        erreur = ft.Text("", color="red")

        def valider(e):
            try:
                poids_clean = poids.value.replace(",", ".")
                taille_clean = taille.value.replace(",", ".")
                profil["prenom"] = prenom.value
                profil["age"] = int(age.value)
                profil["poids"] = float(poids.value)
                profil["taille"] = float(taille.value)
                profil["activites"] = []
                profil["genre"] = genre.value
                sauvegarder_profil(profil)
                ecran_choix()
            except ValueError:
        ...
        changer_ecran(ft.Column(
            [
                ...
                prenom, age, poids, taille, genre, erreur,
                btn("Valider", valider),
            ],
            alignment=ft.MainAxisAlignment.CENTER,
            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
